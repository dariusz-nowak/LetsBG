...
<div class="library flex flex-wrap justify-around">
  ...
  <div class="relative mb-4 flex flex-col shadow-lg @foreach ($game->users as $user) {{ !$user->pivot->favorite ? (!$user->pivot->hidden ? '' : 'order-last') : 'order-first'}} @endforeach" style="flex-basis:24%">
    ...
    <div class="flex">
      <form action="{{ route('library.checkGameStatus', ['game' => $game]) }}" method="post" class="basis-1/6 bg-gray-50 hover:bg-gray-700 transition-all">
        @csrf
        <button class="block w-full py-2">
            <svg class="relative left-1/2 -translate-x-1/2 h-6 w-6 @foreach ($game->users as $user) {{ !$user->pivot->favorite ? 'text-black' : 'text-yellow-500'}} @endforeach"  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <path d="M12 17.75l-6.172 3.245 1.179-6.873-4.993-4.867 6.9-1.002L12 2l3.086 6.253 6.9 1.002-4.993 4.867 1.179 6.873z" /></svg>
        </button>
      </form>
      ...
      <form action="" method="post" class="basis-1/6 bg-gray-50 hover:bg-gray-700 hover:text-white transition-all">
        <button class="w-full block py-2">
          <svg class="relative left-1/2 -translate-x-1/2 h-6 w-6 @foreach ($game->users as $user) {{ !$user->pivot->hidden ? 'text-black' : 'text-red-500'}} @endforeach"  fill="none" viewBox="0 0 24 24" stroke="currentColor">
